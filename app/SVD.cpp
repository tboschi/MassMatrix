...
int main(int argc, char** argv)
{
	...
	while (nIter < nMAX)
	{
		...
		Eigen::MatrixXcd S(nD, nD);
		Eigen::MatrixXcd UMV(nD, nD);
		Eigen::MatrixXcd VMV(nD, nD);
		Eigen::MatrixXcd VMMV(nD, nD);

		S = svd.singularValues().asDiagonal();
		UMV = svd.matrixU().adjoint() * M * svd.matrixV();
		VMV = svd.matrixV().transpose() * M * svd.matrixV();
		VMMV = svd.matrixV().adjoint() * M.adjoint() * M * svd.matrixV();
